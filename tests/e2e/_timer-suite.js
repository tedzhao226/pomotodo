// Generated from the original tests/e2e_timer.js. The timer suite is the most
// internal-coupled set: it drives the state machine and the app's pure globals
// (timerIsPaused, updateTabTitle, formatTime), which app.js exposes only in-page.
// It runs in-page via page.evaluate (see timer.spec.js), preserving all original
// checks; the spec asserts the per-check report.
export async function timerSuite() {
  ...
  return { passed, failedCount: failed.length, failed: failed.map((r) => r.name) };
}
